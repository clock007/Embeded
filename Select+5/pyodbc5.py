...
#conn = pyodbc.connect('DRIVER=FreeTDS;trusted_connection=tcon;port=1433; SERVER=192.168.0.74\\SQL2016R2;UID=sa;PWD=Azarakhsh@Ws;database=Web_Hr;TDS_Version=7.4')
...
conn = pyodbc.connect('DRIVER=FreeTDS; SERVER=192.168.0.78\\SQL2017R2;UID=sa;PWD=Azarakhsh@Ws;database=Web_Hr')
cursor = conn.cursor()
...
cardIdParameter=18
restaurantIpParameter='192.168.0.152'
...
def TokenLisFunc(mealIdparam,MemberOfRestaurantIdparam,current_Dateparam,printedparam,cursorparam) :
   queryTokenList=('select count(t.FoodCalenderId), t.FoodCalenderId ,t.ReserveDate,t.DayName,cast(t.Price as bigint),\
     ...
     where fc.MealId = ' + str(mealIdparam) + ' and t.MembersOfRestaurantId = ' + str(MemberOfRestaurantIdparam) + 'and \
...
query=('select * from Food.Machine m inner join Food.MachinesOfRestaurant mor on m.machineid=mor.machineid  inner join food.Restaurants r on mor.RestaurantsId=r.RestaurantsId where m.IpAdress='+'\''+ restaurantIpParameter +'\'')
...
if cursor.rowcount==0 : # no record in machineOfRestaurant
   ...
   cursor.execute('SELECT * FROM Food.MembersOfRestaurant where CardNo='+str(cardIdParameter)+'and RestaurantsId ='+'\''+str(machineOfRestaurant[14])+'\'') #[Id],[ResturantsId],[NationalNo],[Accept],[Reject],[MemberTypeId],[ReceiptNumber],[DateOfPayment],[CardNumber]
   if cursor.rowcount==0 : 
